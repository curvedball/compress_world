...
static dictItem ZDICT_analyzePos(BYTE* doneMarks, const int* suffix, U32 start, const void* buffer, U32 minRatio, U32 notificationLevel)
{
...

    {
        ...
        for (i=LLIMIT-1; i>=MINMATCHLENGTH; i--) if (cumulLength[i]>=minRatio)
            {
                break;
            }
        maxLength = i;
...
static size_t ZDICT_trainBuffer_legacy(dictItem* dictList, U32 dictListSize,
                                       ...
                                       const size_t* fileSizes, unsigned nbFiles, U32 minRatio, U32 notificationLevel)
{
...
static void ZDICT_countEStats(EStats_ress_t esr, ZSTD_parameters params, U32* countLit, U32* offsetcodeCount, U32* matchlengthCount, U32* litlengthCount, U32* repOffsets, const void* src, size_t srcSize, U32 notificationLevel)
{
...
//
static size_t ZDICT_analyzeEntropy(void*  dstBuffer, size_t maxDstSize, unsigned compressionLevel, const void*  srcBuffer, const size_t* fileSizes, unsigned nbFiles, const void* dictBuffer, size_t  dictBufferSize,  unsigned notificationLevel)
{
    U32 countLit[256];
    ...
    {
        eSize = ERROR(dictionaryCreation_failed);    /* too large dictionary */
        goto _cleanup;
    }
    ...
    for (u=0; u<nbFiles; u++)
    {
        ZDICT_countEStats(esr, params, countLit, offcodeCount, matchLengthCount, litLengthCount, repOffset, (const char*)srcBuffer + pos, fileSizes[u], notificationLevel);
        pos += fileSizes[u];
    }
...
#define HBUFFSIZE 256   /* should prove large enough for all entropy headers */
    ...
    {
		DbgPrint("zb===ZDICT_finalizeDictionary===1.0===\n\n");
	...
	//
    {
        size_t const eSize = ZDICT_analyzeEntropy(header+hSize, HBUFFSIZE-hSize, compressionLevel, samplesBuffer, samplesSizes, nbSamples, customDictContent, dictContentSize, notificationLevel);
...
size_t ZDICT_addEntropyTablesFromBuffer_advanced(void* dictBuffer, size_t dictContentSize, size_t dictBufferCapacity, const void* samplesBuffer, const size_t* samplesSizes, unsigned nbSamples, ZDICT_params_t params)
{
    int const compressionLevel = (params.compressionLevel == 0) ? g_compressionLevel_default : params.compressionLevel;
    U32 const notificationLevel = params.notificationLevel;
    size_t hSize = 8;
    ...
    DISPLAYLEVEL(2, "statistics ... \n");
    {
        size_t const eSize = ZDICT_analyzeEntropy((char*)dictBuffer+hSize, dictBufferCapacity-hSize, compressionLevel, samplesBuffer, samplesSizes, nbSamples, (char*)dictBuffer + dictBufferCapacity - dictContentSize, dictContentSize, notificationLevel);
        if (ZDICT_isError(eSize))
        {
            return eSize;
        }
    ...
    {
		DbgPrint("zb===ZDICT_addEntropyTablesFromBuffer_advanced===============1.0===\n\n");
...
*/
size_t ZDICT_trainFromBuffer_unsafe_legacy(void* dictBuffer, size_t maxDictSize, const void* samplesBuffer, const size_t* samplesSizes, unsigned nbSamples, ZDICT_legacy_params_t params)
{
    ...
    {
        free(dictList);    /* requested dictionary size is too small */
        return ERROR(dstSize_tooSmall);
    }
    if (samplesBuffSize < ZDICT_MIN_SAMPLES_SIZE)
    {
        free(dictList);    /* not enough source to create dictionary */
    ...
    if (params.zParams.notificationLevel>= 3)
    {
        ...
        for (u=1; u<nb; u++)
        {
            ...
            DISPLAYLEVEL(3, "%3u:%3u bytes at pos %8u, savings %7u bytes |", u, length, pos, dictList[u].savings);
    ...
    /* create dictionary */
    {
        ...
        if (dictContentSize < ZDICT_CONTENTSIZE_MIN)
        {
            free(dictList);    /* dictionary content too small */
            return ERROR(dictionaryCreation_failed);
        }
        ...
        /* build dict content */
        {
            ...
            for (u=1; u<dictList->pos; u++)
            {
                ...
                {
                    free(dictList);    /* should not happen */
