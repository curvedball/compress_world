...
{
    size_t const startingInputLength = (format == ZSTD_f_zstd1_magicless) ? ZSTD_frameHeaderSize_prefix - ZSTD_frameIdSize : ZSTD_frameHeaderSize_prefix;
...
size_t ZSTD_getFrameHeader_advanced(ZSTD_frameHeader* zfhPtr, const void* src, size_t srcSize, ZSTD_format_e format)
{
	...
	DbgPrint("zb===ZSTD_getFrameHeader_advanced===1.0===minInputSize: %d srcSize: %d\n", minInputSize, srcSize);
    if (srcSize < minInputSize)
    {
		DbgPrint("zb===ZSTD_getFrameHeader_advanced===1.0a===\n");
        return minInputSize;
    }
    ...
    /* ensure there is enough `srcSize` to fully read/decode frame header */
    {
		...
		//
		DbgPrint("zb===ZSTD_getFrameHeader_advanced===1.1===srcSize: %d fhsize: %d\n", srcSize, fhsize);
		
		//
        if (srcSize < fhsize)
        {
			DbgPrint("zb===ZSTD_getFrameHeader_advanced===1.1a===\n");
            return fhsize;
        }
	...
	//=====================================
    {
		...
		DbgPrint("zb===ZSTD_getFrameHeader_advanced===2.0===fhdByte: 0x%.2x dictIDSizeCode: %d singleSegment: %d fcsID: %d\n", fhdByte, dictIDSizeCode, singleSegment, fcsID);
		...
		//zb: fcsID determines the bit width of the frame_length field. 
        switch(fcsID)
        {
            default:
                assert(0);  /* impossible */
		...
		//
		DbgPrint("zb===ZSTD_getFrameHeader_advanced===3.0===pos: %d frameContentSize: %d, 0x%.8x\n", pos, frameContentSize, ip[pos]);
...
size_t ZSTD_getcBlockSize(const void* src, size_t srcSize, blockProperties_t* bpPtr)
{
	...
	//
    {
        ...
        U32 const cSize = cBlockHeader >> 4; //zbdelta
		//
        bpPtr->lastBlock = cBlockHeader & 1;
        bpPtr->blockType = (blockType_e)((cBlockHeader >> 1) & 3);
		bpPtr->deltaFlag = (cBlockHeader >> 3) & 1;
        bpPtr->origSize = cSize;   /* only useful for RLE */
		
		DbgPrint("ZSTD_getcBlockSize==cBlockHeader: 0x%.8x=======blockType: %d lastBlock: %d cSize: %d\n", cBlockHeader, bpPtr->blockType, bpPtr->lastBlock, cSize);
            ...
            case set_compressed:
                ...
                }
                {
                    ...
                    if (HUF_isError((litEncType==set_repeat) ?
                                    ...
                                    ( singleStream ?
                                      HUF_decompress1X1_DCtx_wksp_bmi2(dctx->entropy.hufTable, dctx->litBuffer, litSize, istart+lhSize, litCSize, dctx->entropy.workspace, sizeof(dctx->entropy.workspace), dctx->bmi2) :
                                      HUF_decompress4X_hufOnly_wksp_bmi2(dctx->entropy.hufTable, dctx->litBuffer, litSize, istart+lhSize, litCSize, dctx->entropy.workspace, sizeof(dctx->entropy.workspace), dctx->bmi2))))
                    {
                        return ERROR(corruption_detected);
                    }
...
static size_t ZSTD_buildSeqTable(ZSTD_seqSymbol* DTableSpace, const ZSTD_seqSymbol** DTablePtr, symbolEncodingType_e type, U32 max, U32 maxLog, const void* src, size_t srcSize, const U32* baseValue, const U32* nbAdditionalBits, const ZSTD_seqSymbol* defaultTable, U32 flagRepeatTable)
{
    switch(type)
    {
        case set_rle :
...
static const U32 LL_base[MaxLL+1] =  /*zb@:  */
{
    0,    1,    2,     3,     4,     5,     6,      7,
...
static const U32 OF_base[MaxOff+1] = /*zb@: */
{
...
static const U32 ML_base[MaxML+1] = /*zb@:  */
{
    3,  4,  5,    6,     7,     8,     9,    10,
...
size_t ZSTD_decodeSeqHeaders(ZSTD_DCtx* dctx, int* nbSeqPtr, const void* src, size_t srcSize)
{
    ...
    }
    {
        ...
        {
			//
            size_t const llhSize = ZSTD_buildSeqTable(dctx->entropy.LLTable, &dctx->LLTptr, LLtype, MaxLL, LLFSELog, ip, iend-ip, LL_base, LL_bits, LL_defaultDTable, dctx->fseEntropy);
        ...
        {
			//
            size_t const ofhSize = ZSTD_buildSeqTable(dctx->entropy.OFTable, &dctx->OFTptr, OFtype, MaxOff, OffFSELog, ip, iend-ip, OF_base, OF_bits, OF_defaultDTable, dctx->fseEntropy);
        ...
        {
			//
            size_t const mlhSize = ZSTD_buildSeqTable(dctx->entropy.MLTable, &dctx->MLTptr, MLtype, MaxML, MLFSELog, ip, iend-ip, ML_base, ML_bits, ML_defaultDTable, dctx->fseEntropy);
...
HINT_INLINE size_t ZSTD_execSequence(BYTE* op, BYTE* const oend, seq_t sequence,  const BYTE** litPtr, const BYTE* const litLimit, const BYTE* const prefixStart, const BYTE* const virtualStart, const BYTE* const dictEnd)
{
    ...
    {
        ZSTD_wildcopy(op + 8, (*litPtr) + 8, sequence.litLength - 8);    /*zb:  note : since oLitEnd <= oend-WILDCOPY_OVERLENGTH, no risk of overwrite beyond oend */
    }
    ...
    *litPtr = iLitEnd;   /* update for next sequence */
	...
	//================================================
    /* copy Match */
    if (sequence.offset > (size_t)(oLitEnd - prefixStart)) //zb@:
    {
    ...
    if (oMatchEnd > oend - (16-MINMATCH))
    {
        if (op < oend_w)
        {
            ZSTD_wildcopy(op, match, oend_w - op); //zb:
            match += oend_w - op;
            op = oend_w;
        }
        while (op < oMatchEnd)
    ...
    {
        ZSTD_wildcopy(op, match, (ptrdiff_t)sequence.matchLength - 8);   /*zb:  works even if matchLength < 8 */
    }
    return sequenceLength;
}
...
FORCE_INLINE_TEMPLATE seq_t ZSTD_decodeSequence(seqState_t* seqState, const ZSTD_longOffset_e longOffsets)
{
	...
	//zb:
	//
    seq.litLength = llBase + ((llBits > 0) ? BIT_readBitsFast(&seqState->DStream, llBits/*>0*/) : 0);    /*zb? :   <=  16 bits */
    if (MEM_32bits())
    {
        BIT_reloadDStream(&seqState->DStream);
    }
	...
	//zb:
	//
    ZSTD_updateFseState(&seqState->stateOffb, &seqState->DStream);  /* <=  8 bits */ //zb: 
...
static void ZSTD_DeltaDecodingLiterals(const void* src, size_t srcSize)
{
#if 1
...
FORCE_INLINE_TEMPLATE size_t ZSTD_decompressSequences_body( ZSTD_DCtx* dctx, void* dst, size_t maxDstSize, const void* seqStart, size_t seqSize, int nbSeq, const ZSTD_longOffset_e isLongOffset)
{
	...
	if (dctx->deltaFlag)
	{
		ZSTD_DeltaDecodingLiterals(litPtr, dctx->litSize); //zbdelta
	}
    ...
    if (nbSeq)
    {
        ...
        for ( ; (BIT_reloadDStream(&(seqState.DStream)) <= BIT_DStream_completed) && nbSeq ; )
        {
            ...
            {
				//
                seq_t const sequence = ZSTD_decodeSequence(&seqState, isLongOffset); //zbd:
				...
				//
                op += oneSeqSize; //zb: decompress data has already been put into the output buffer.
            }
        }
...
static TARGET_ATTRIBUTE("bmi2") size_t ZSTD_decompressSequences_bmi2(ZSTD_DCtx* dctx, void* dst, size_t maxDstSize, const void* seqStart, size_t seqSize, int nbSeq, const ZSTD_longOffset_e isLongOffset)
{
    return ZSTD_decompressSequences_body(dctx, dst, maxDstSize, seqStart, seqSize, nbSeq, isLongOffset);
}


static TARGET_ATTRIBUTE("bmi2") size_t ZSTD_decompressSequencesLong_bmi2(ZSTD_DCtx* dctx, void* dst, size_t maxDstSize, const void* seqStart, size_t seqSize, int nbSeq, const ZSTD_longOffset_e isLongOffset)
{
    return ZSTD_decompressSequencesLong_body(dctx, dst, maxDstSize, seqStart, seqSize, nbSeq, isLongOffset);
}
...
static size_t ZSTD_decompressBlock_internal(ZSTD_DCtx* dctx, void* dst, size_t dstCapacity, const void* src, size_t srcSize, const int frame)
{
    ...
    /* Decode literals section */
    {
        ...
        DEBUGLOG(5, "ZSTD_decodeLiteralsBlock_func : %u", (U32)litCSize);
    ...
    /* Build Decoding Tables */
    {
        ...
        if ( (!frame || dctx->fParams.windowSize > (1<<24)) && (nbSeq>0) )    /* could probably use a larger nbSeq limit */
        {
		...
		//
        return ZSTD_decompressSequences(dctx, dst, dstCapacity, ip, srcSize, nbSeq, isLongOffset); //zbd:
    }
}
...
size_t ZSTD_findFrameCompressedSize(const void *src, size_t srcSize)
{
    ...
    else
    {
		...
		DbgPrint("srcSize: %d headerSize: %d remainingSize: %d\n", srcSize, zfh.headerSize, remainingSize);
        ...
        while (1)
        {
			...
			//DbgPrint("cBlockSize: %d remainingSize: %d\n", cBlockSize, remainingSize);
			
            if (ZSTD_isError(cBlockSize))
            {
				//DbgPrint("ZSTD_findFrameCompressedSize==1.0a\n");
                return cBlockSize;
            }

            if (ZSTD_blockHeaderSize + cBlockSize > remainingSize)
            {
				DbgPrint("ZSTD_findFrameCompressedSize==1.0b\n");
                return ERROR(srcSize_wrong);
            }
			...
			//DbgPrint("ip: 0x%.8x ipstart: 0x%.8x\n", ip, ipstart);

            if (blockProperties.lastBlock)
            {
				DbgPrint("ZSTD_findFrameCompressedSize==1.0c\n");
                break;
            }
        }
...
static size_t ZSTD_decompressFrame(ZSTD_DCtx* dctx, void* dst, size_t dstCapacity, const void** srcPtr, size_t *srcSizePtr)
{
    ...
    while (1)
    {
        ...
        {
            case bt_compressed:
				dctx->deltaFlag = blockProperties.deltaFlag; //zbdelta
...
size_t ZSTD_decompressContinue(ZSTD_DCtx* dctx, void* dst, size_t dstCapacity, const void* src, size_t srcSize)
{
    ...
    switch (dctx->stage)
    {
        ...
        case ZSTDds_decodeBlockHeader:
        {
            ...
            dctx->expected = cBlockSize; //zb: BlockHeadSize is assigned to this variable.
            ...
            DEBUGLOG(5, "ZSTD_decompressContinue: case ZSTDds_decompressBlock");
            {
                ...
                switch(dctx->bType)
                {
                    case bt_compressed:
						...
						//zb: 
						//
                        rSize = ZSTD_decompressBlock_internal(dctx, dst, dstCapacity, src, srcSize, /* frame */ 1); //zbd:
                        break;
...
static size_t ZSTD_loadEntropy(ZSTD_entropyDTables_t* entropy, const void* const dict, size_t const dictSize)
{
    ...
    {
        size_t const hSize = HUF_readDTableX2_wksp(entropy->hufTable, dictPtr, dictEnd - dictPtr,  entropy->workspace, sizeof(entropy->workspace));
...
size_t ZSTD_decompressBegin_usingDDict(ZSTD_DCtx* dstDCtx, const ZSTD_DDict* ddict)
{
	...
	DbgPrint("zb===ZSTD_decompressBegin_usingDDict==1.0==\n");
	//
    if (ddict)     /* support begin on NULL */
    {
		DbgPrint("zb===ZSTD_decompressBegin_usingDDict==1.1==\n");
        ...
        {
			DbgPrint("zb===ZSTD_decompressBegin_usingDDict==1.2a==\n");
        ...
        {
			DbgPrint("zb===ZSTD_decompressBegin_usingDDict==1.2b==\n");
...
size_t ZSTD_decompressStream(ZSTD_DStream* zds, ZSTD_outBuffer* output, ZSTD_inBuffer* input)
{
	...
	//
	DbgPrint("zb===ZSTD_decompressStream===1.0===zds->streamStage: %d zds->lhSize: %d zds->format: %d\n", zds->streamStage, zds->lhSize, zds->format);

	//
    while (someMoreWork) /* task_loop of ZSTD decompress,  zbzbzbzbzb */
    {
		DbgPrint("while_loop===zds->streamStage: %d\n", zds->streamStage);
        switch(zds->streamStage)
        {
            ...
            case zdss_loadHeader:
                ...
                {
					//
					//zb: read the frame_length field.
					//
                    ...
                    if (hSize != 0)     /* need more input */
                    {
						...
						//zb: 
						//
                        memcpy(zds->headerBuffer + zds->lhSize, ip, toLoad); //zb: Put the frame header(5 bytes) into the buffer
				...
				//
				DbgPrint("zb===ZSTD_decompressStream===1.1===zds->fParams.frameContentSize: %d zds->fParams.windowSize: %d\n", zds->fParams.frameContentSize, zds->fParams.windowSize);
				
                /* check for single-pass mode opportunity */
                if (zds->fParams.frameContentSize && zds->fParams.windowSize /* skippable frame if == 0 */ && (U64)(size_t)(oend - op) >= zds->fParams.frameContentSize)
                {
					DbgPrint("zb===ZSTD_decompressStream===1.2===\n");
					//
					//zb: 
					//
					...
					//
					DbgPrint("zb===ZSTD_decompressStream===1.3===cSize: %d\n", cSize);

					//
                    if (cSize <= (size_t)(iend - istart))
                    {
						DbgPrint("zb===ZSTD_decompressStream===1.4===\n");
						
                        /* shortcut : using single-pass mode */
                        size_t const decompressedSize = ZSTD_decompress_usingDDict(zds, op, oend-op, istart, cSize, zds->ddict);

						//
						DbgPrint("zb===ZSTD_decompressStream===1.5===decompressedSize: %d\n", decompressedSize);
                ...
                else
                {
                    ...
                    zds->expected = ZSTD_blockHeaderSize; //zb:  value=3
                    zds->stage = ZSTDds_decodeBlockHeader; //zb: 
                }
            ...
            case zdss_read:
				...
				//
                {
                    size_t const neededInSize = ZSTD_nextSrcSizeToDecompress(zds); //zb: 
					...
					DbgPrint("continue_do_read. (iend - ip)=%d neededInSize=%d\n", (iend - ip), neededInSize);
                    if ((size_t)(iend - ip) >= neededInSize)    /* decode directly from src */
                    {
						DbgPrint("buffer_greater\n");
                        int const isSkipFrame = ZSTD_isSkipFrame(zds);

						//
						//zb: 
						//
                        size_t const decodedSize = ZSTD_decompressContinue(zds, zds->outBuff + zds->outStart, (isSkipFrame ? 0 : zds->outBuffSize - zds->outStart), ip, neededInSize);
						DbgPrint("Now decodedSize: %d\n", decodedSize);
                ...
                if (ip == iend)
                {
					DbgPrint("no_more_input\n");
                    someMoreWork = 0;    /* no more input */
                    break;
                }

				DbgPrint("runt_into_next_stage...\n");
            ...
            case zdss_load:
            {
                size_t const neededInSize = ZSTD_nextSrcSizeToDecompress(zds); 	//zb: the current processing pos  of the input buffer 
                size_t const toLoad = neededInSize - zds->inPos; 				//zb: the number of bytes to read in current process.
                int const isSkipFrame = ZSTD_isSkipFrame(zds);
                size_t loadedSize;

				DbgPrint("in_the_stage===neededInSize: %d toLoad: %d isSkipFrame: %d\n", neededInSize, toLoad, isSkipFrame);
                if (isSkipFrame)
                {
                ...
                else
                {
                    ...
                    {
						DbgPrint("runt_into_corruption\n");
                        return ERROR(corruption_detected);    /* should never happen */
                    }

					//
					DbgPrint("(iend - ip): %d zds->inPos: %d\n", iend - ip, zds->inPos);
                    loadedSize = ZSTD_limitCopy(zds->inBuff + zds->inPos, toLoad, ip, iend - ip);
					DbgPrint("loadedSize: %d\n", loadedSize);
                }
                ...
                {
					DbgPrint("no_more_input__wait_for_more...\n");
                    someMoreWork = 0;    /* not enough input, wait for more */
                    break;
                }

				DbgPrint("more_input_continue\n");
                ...
                /* decode loaded input */
                {
					...
					//zb: 
					//
                    size_t const decodedSize = ZSTD_decompressContinue(zds, zds->outBuff + zds->outStart, zds->outBuffSize - zds->outStart, zds->inBuff, neededInSize);
					DbgPrint("decode_loaded_input===decodedSize: %d\n", decodedSize);
					//
                    if (ZSTD_isError(decodedSize))
                    {
                        return decodedSize;
	...
	//
    {
		...
		DbgPrint("zb===now===nextSrcSizeHint: %d\n", nextSrcSizeHint);
