...
zb:
	save the frequency of each huffman coding width to the output buffer (Used to construct the huffman tree when decompress).
 */
...
/*
zb: 
	count: the frequency of each literals has been saved in this buffer.
*/
size_t HUF_buildCTable_wksp (HUF_CElt* tree, const U32* count, U32 maxSymbolValue, U32 maxNbBits, void* workSpace, size_t wkspSize)
{
	...
	DbgPrint("zb===HUF_buildCTable_wksp===maxSymbolValue: %d maxNbBits: %d\n", maxSymbolValue, maxNbBits);

    /* sort, decreasing order */
    HUF_sort(huffNode, count, maxSymbolValue); //zb?
    ...
    {
        nonNullRank--; //zb: Move backward to find the pos that the count is not 0.
    }
    ...
    /* fill result into tree (val, nbBits) */
    {
		...
		//zb: save the val and bits of each character. Get huffman prefix code cell. 
        for (n = 0; n <= maxSymbolValue; n++)
        {
...
zb:
	dst:
	dstSize
	src:  the literal buffer of LZ algorithm.
	srcSize: 
	CTable: Save the frequency of each bit width of huffman coding

Task:
	Huffman coding all the literals to the output buffer.
*/
FORCE_INLINE_TEMPLATE size_t HUF_compress1X_usingCTable_internal_body(void* dst, size_t dstSize, const void* src, size_t srcSize, const HUF_CElt* CTable)
{
    ...
    {
        HUF_encodeSymbol(&bitC, ip[n - 1], CTable);		//zb: bitC->bitContainer is used to save the huffman coding bits temprarily!
        HUF_FLUSHBITS_1(&bitC); 						//zb: save it to output buffer(bitC->ptr points the current pos of output buffer).
...
static TARGET_ATTRIBUTE("bmi2") size_t HUF_compress1X_usingCTable_internal_bmi2(void* dst, size_t dstSize, const void* src, size_t srcSize, const HUF_CElt* CTable)
{
...
{
    size_t const cSize = singleStream ? HUF_compress1X_usingCTable_internal(op, oend - op, src, srcSize, CTable, bmi2) : HUF_compress4X_usingCTable_internal(op, oend - op, src, srcSize, CTable, bmi2);
 ...
 * `workSpace` must a table of at least HUF_WORKSPACE_SIZE_U32 unsigned
 ...
 zb:
 	dst:
 	dstSize: 
 	src: points to the literal buffer
 	srcSize: the literals length
 	maxSymbolValue = 255,
 	huffLog = 11,
 	singleStream=1 (1X stream) and  singleStream=0 (4X streams)
...
static size_t HUF_compress_internal (void* dst, size_t dstSize, const void* src, size_t srcSize, unsigned maxSymbolValue, unsigned huffLog, unsigned singleStream, void* workSpace, size_t wkspSize, HUF_CElt* oldHufTable, HUF_repeat* repeat, int preferRepeat, const int bmi2)
{
	...
	DbgPrint("zb===HUF_compress_internal===1.0===\n");
	...
	//
	DbgPrint("zb===HUF_compress_internal===2.0===\n");

	//===================================================================
    /* Heuristic : If old table is valid, use it for small inputs */
    if (preferRepeat && repeat && *repeat == HUF_repeat_valid)
    {
		DbgPrint("zb===HUF_compress_internal===2.0a===\n");
        return HUF_compressCTable_internal(ostart, op, oend, src, srcSize, singleStream, oldHufTable, bmi2);
    }

    /* Scan input and build symbol stats */
    {
		DbgPrint("zb===HUF_compress_internal===2.1===\n");
		//
		//zb: Caculate the freqency of each character in the literal buffer.
		//
		...
		DbgPrint("zb===HUF_compress_internal===2.2===largest: %d repeatVal: %d\n", largest, *repeat);
    }
    ...
    {
		DbgPrint("zb===HUF_compress_internal===2.3a===\n");
    ...
    {
		DbgPrint("zb===HUF_compress_internal===2.3b===\n");
        return HUF_compressCTable_internal(ostart, op, oend, src, srcSize, singleStream, oldHufTable, bmi2);
    }

	//
	DbgPrint("zb===HUF_compress_internal===3.0===\n");
	...
	//
	DbgPrint("zb===HUF_compress_internal===3.1===huffLog: %d\n", huffLog);

	//
    {
		//
		//zb: ----------Save each code cell of huffman for each character!------------
		//
        CHECK_V_F(maxBits, HUF_buildCTable_wksp(table->CTable, table->count, maxSymbolValue, huffLog, table->nodeTable, sizeof(table->nodeTable)) );
    ...
    {
		//
		//zb: ---save the frequency of each huffman coding width to the output buffer (HuffmanTable_Literals_HeadData) ----------
		//
        ...
        {
			DbgPrint("zb===HUF_compress_internal===3.1a===\n");
            return 0;
        }
        ...
        if (repeat)
        {
			DbgPrint("zb===HUF_compress_internal===3.1b===\n");
            *repeat = HUF_repeat_none;
        }
		
        if (oldHufTable)
        {
			DbgPrint("zb===HUF_compress_internal===3.1c===\n");
            memcpy(oldHufTable, table->CTable, sizeof(table->CTable));    /* Save new table */
        }
    }

	//
	//zb: ----Huffman coding all the literals to the output buffer (HuffmanTable_Literals_ContentData)----------------
	//
...
{
    return HUF_compress_internal(dst, dstSize, src, srcSize, maxSymbolValue, huffLog, 1, workSpace, wkspSize, NULL, NULL, 0, 0);
}
...
{
    return HUF_compress_internal(dst, dstSize, src, srcSize, maxSymbolValue, huffLog, 1, workSpace, wkspSize, hufTable, repeat, preferRepeat, bmi2);
}
...
{
    return HUF_compress_internal(dst, dstSize, src, srcSize, maxSymbolValue, huffLog, 0 /*4 streams*/, workSpace, wkspSize, NULL, NULL, 0, 0);
}
